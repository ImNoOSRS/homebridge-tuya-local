...
class SimpleDimmerAccessory extends BaseAccessory {
    ...
    _registerPlatformAccessory() {
        ...
        const lightService = this.accessory.addService(Service.Lightbulb, this.device.context.name);
        if (lightService && typeof lightService.setPrimaryService === 'function') {
    ...
    _registerCharacteristics(dps) {
        ...
        this._wledCurrentEffectId = null;
        ...
        if (Array.isArray(presetEffects) && presetEffects.length) {
            ...
            presetEffects.forEach((effectCfg, index) => {
                ...
                const effectName =
                    (effectCfg.displayName && String(effectCfg.displayName).trim()) ||
                    (effectCfg.name && String(effectCfg.name).trim()) ||
                    `Effect ${index + 1}`;

                // Use the effect name as the switch name; users can still rename from HomeKit.
                ...
                const onCharacteristic = effectService
                    ...
                    .on('set', (value, callback) => {
                        ...
                        // If the Tuya dimmer itself is OFF, do not attempt to change WLED.
                        if (!this.device.state[this.dpPower]) {
                            this.log.info(
                                '[WLED Sync] %s: ignoring preset effect "%s" (id=%s) because dpPower (%s) is OFF',
                                this.device.context.name,
                                effectName,
                                effectId,
                                this.dpPower
                            ...
                            // Reflect that the effect is not actually active.
                            if (onCharacteristic.value) {
                        ...
                        this.log.info(
                            '[WLED Sync] %s: setting preset effect "%s" (id=%s)',
                            this.device.context.name,
                            effectName,
                            effectId
                        );

                        this._setWledEffect(effectId, err => {
                            if (err) {
                                this.log.error(
                            ...
                            this._wledCurrentEffectId = effectId;
                            this._wledEffectSwitches.forEach(sw => {
                                if (sw.effectId !== effectId && sw.characteristic.value) {
                    ...
                    .on('get', cb => {
                        cb(null, this._wledCurrentEffectId === effectId);
                    });
    ...
    _setWledEffect(effectId, callback) {
        const target = this._getWledTarget();
        if (!target) {
            this.log.error(
        ...
        // If Tuya power is off, do not attempt to talk to WLED for effects.
        if (!this.device.state[this.dpPower]) {
            this.log.info(
                '[WLED Sync] %s: _setWledEffect(%s) skipped because dpPower (%s) is OFF',
                this.device.context.name,
                effectId,
                this.dpPower
            );
        ...
        // WLED expects the effect index in the "fx" field of the first segment.
        const body = JSON.stringify({seg: [{fx: effectId}]});
