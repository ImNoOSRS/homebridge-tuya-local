...
class SimpleDimmerAccessory extends BaseAccessory {
    ...
    getBrightness(callback) {
        if (this.syncBrightnessToWled) {
            ...
            // No cached value yet: return a sane default immediately and refresh WLED in the background.
            const defaultValue = this.device.state[this.dpPower] ? 100 : 0;
            this.log.info(
                '[WLED Sync] %s: getBrightness() -> returning default %s%% and refreshing WLED in background',
                this.device.context.name,
                defaultValue
            );

            // Fire-and-forget WLED query so Homebridge is not blocked.
            setImmediate(() => {
                this._getWledBrightness((err, bri) => {
                    if (err || !isFinite(bri)) {
                        this.log.error(
                            '[WLED Sync] %s: background getBrightness WLED error=%s',
                            this.device.context.name,
                            err
                        );
                        return;
                    ...
                    const value = Math.max(0, Math.min(100, Math.round((bri / maxWledBrightness) * 100)));
                    this.log.info(
                        '[WLED Sync] %s: background getBrightness WLED bri=%s -> %s%%',
                        this.device.context.name,
                        bri,
                        value
                    );
                    this._lastWledPercent = value;
                    if (this._characteristicBrightness) {
                        this._characteristicBrightness.updateValue(value);
                    }
                });
            });
